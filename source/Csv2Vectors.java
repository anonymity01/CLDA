...
public class Csv2Vectors {
	...
	static CommandOption.Integer min_sup = new CommandOption.Integer(Csv2Vectors.class, "min_sup", "INTEGER", true, 10,
			 "Find rare words.\n" +
	         "   Find rare words.", null);
	...
	public static void main (String[] args) throws FileNotFoundException, IOException {
		...
		if (usePipeFromVectorsFile.wasInvoked()) {
			...
			pipeList.add( new EnglishStemming(null, counter));
		...
		Writer rareWriter = new OutputStreamWriter(new FileOutputStream(new File("data/rarewords")));
        //write the rare words, and they are used as stop words later
        for(String word : counter.keySet()){
        	int tmpCnt = counter.get(word);
        	if(tmpCnt < min_sup.value){
		...
		if (usePipeFromVectorsFile.wasInvoked()) {
			...
			if (stoplistFile.wasInvoked()) {
				...
				// The user specified a new list
				pipeList.add( new EnglishStemming(voc));
				TokenSequenceRemoveStopwords stopwordFilter =
					new TokenSequenceRemoveStopwords(stoplistFile.value,
													 encoding.value,
													 false, // don't include default list
		...
		Writer stemWriter = new OutputStreamWriter(new FileOutputStream(new File("data/steammaping")));
		String vocFile = "data/vocabulary"; // this is the file for labels
		...
		//LabelAlphabet topicAlphabet = AlphabetFactory.labelAlphabetOfSize(numTopics);
		//tokensPerTopic = new int[numTopics];
        int indw=0;
//        while(itr.hasNext()){
//        	String key = (String)itr.next();
//        	//write to the vocabulary
//        	vocWriter.write(indw+":"+key+"\n");
//        	vocabulary.put(key, indw);
//        	indw++;
//        }
        while(itr.hasNext()){
        	String key = (String)itr.next();
        ...
        vocWriter.close();
        stemWriter.close();
        String dataFile = "data/data"; // this is the file for labels
		Writer dataWriter = new OutputStreamWriter(new FileOutputStream(new File(dataFile)));
        for (Instance instance : instances) {
			FeatureSequence tokens = (FeatureSequence) instance.getData();
			for (int position = 0; position < tokens.size(); position++) {
				String type = String.valueOf(tokens.getIndexAtPosition(position));
				if(position==0)
				{
